...
//Основаная функция Л
double f1_lena(double x) { //функция
	return cos(pow(M_E, x));
}
double f1_lena2(double x) { //первая произвоная ф-и
	return -1* pow(M_E, x)*sin(pow(M_E, x));
}
double f1_lena3(double x) { //вторая произвоная ф-и
	return -pow(M_E, x) * sin(pow(M_E, x))- pow(M_E, 2*x) * cos(pow(M_E, x));
}
//Доп функция Л
...
double f1_ver2(double x) {
	return 2*pow(x,3)/sqrt(1 + pow(x, 4));
}
double f1_ver3(double x) {
	return (2*x*x*(x*x*x*x+3))/pow(1 + pow(x, 4), 1.5);
}
//Доп функция В
double f2_ver(double x) {
	return pow(1 + pow(x, 4), 0.5) + cos(10 * x);
}
double f2_ver2(double x) {
	return 2 * pow(x, 3) * pow(1 + pow(x, 4), -0.5) - 10 * sin(10 * x);
}
double f2_ver3(double x) {
	return (6 * x + 6 * pow(x, 5) - pow(x, 3)) * pow(1 + pow(x, 4), -1.5) - 10 * 10 * cos(10 * x);
}
